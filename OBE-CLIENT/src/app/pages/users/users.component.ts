...
export class UsersComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.httpClient.get<{ response: User[], date: Date }>(`${environment.serverUrl}/users?department=${this.dataService.userModel.department}&userId=${this.dataService.userModel._id}`, {headers: this.dataService.httpHeaders})
      .toPromise()
      .then((value) => {
  ...
  submitForm(form: FormGroup) {
    ...
    if (!this.updation) {
      this.httpClient.post<{ res: User, error: any }>(`${environment.serverUrl}/users/add-user`, { ...userObj }, {headers: this.dataService.httpHeaders} )
        .toPromise()
        .then((value) => {
      ...
      this.httpClient.put<{ response: User }>(`${environment.serverUrl}/users/update-user/${userObj._id}`, { ...userObj }, {headers: this.dataService.httpHeaders})
        .toPromise()
        .then((value) => {
          // console.log(":>>> Value: ", value);
          this.loader = false;
  ...
  deleteUser(modalRef: any, id: any) {
    this.modalService.open(modalRef).result.then((value) => {
      this.httpClient.delete(`${environment.serverUrl}/users/delete-user/${id}`, {headers: this.dataService.httpHeaders})
      .toPromise()
      .then((value) => {
