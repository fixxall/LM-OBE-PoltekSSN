...
export class CourseOutcomeComponent implements OnInit, OnDestroy {
  ...
  submitForm(form: FormGroup) {
    ...
    this.httpClient.post<{ response: CourseOutcomes, error: any }>(`${environment.serverUrl}/course-outcomes/add-co`, { ...values },{headers: this.data.httpHeaders})
    .toPromise()
    .then((value) => {
  ...
  submitUpdateCOForm(form: FormGroup) {
    ...
    let values: CourseOutcomes = { ...form.value };
    values.cognitiveDomain = values.cognitiveDomain?.sort();
    this.httpClient.put<{ response: CourseOutcomes }>(`${environment.serverUrl}/course-outcomes/update-co/${form.value._id}`, { ...values },{headers: this.data.httpHeaders})
    .toPromise()
    .then((value) => {
      // console.log(":>>> Value: ", value);
      this.loader = false;
  ...
  deleteCO(modalRef: any, id: any) {
    this.modalService.open(modalRef).result.then((value) => {
      this.httpClient.delete(`${environment.serverUrl}/course-outcomes/delete-co/${id}`,{headers: this.data.httpHeaders})
      .toPromise()
      .then((value) => {
